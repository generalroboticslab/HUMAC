...
from Model.Imitation_learning import CNN, train_with_rotation
...
model = CustomResNet18(num_of_frames,rotation=True)
...
class_weights = torch.tensor([0.0257, 1.6096, 1.3648])
class_weights = class_weights.to('cuda')

criterion2 = nn.CrossEntropyLoss(weight=class_weights)
# optimizer2 = optim.Adam(filter(lambda p: p.requires_grad, model.parameters()), lr=0.001)

save_base_directory = f"Weights/IL_Long_split_by_traj_{num_of_frames}frames_{step_ahead}step_ahead_with_rotation"
...
t_l,v_l = train_with_rotation(model,
          100,
          'cuda',
