...
class HideandSeekDataset(Dataset):
   ...
   def get(self, index):
      ...
      if ind + self.step_ahead >= len(self.position_list):
         action_list.append(self.position_list[-1])
      else:
         action_list.append(self.position_list[ind+self.step_ahead])

      for j in range(len(self.teammate_position_list)):
         if ind + self.step_ahead >= len(self.teammate_position_list[j]):
            action_list.append(self.teammate_position_list[j][-1])
         else:
            action_list.append(self.teammate_position_list[j][ind+self.step_ahead])
      ...
      # print("Action before",action_list)
      # print(index_list)
      ...
      # Calculate the extent of the image to center it at (0, 0)
         ...
         if (i == num_of_frame):
            for j in range(3):
               try:
                  if action[2*j] > -19: 
                     axs[0,i-1].scatter(action[2*j], action[2*j+1], c='orange', marker='+', s=60 - j* 20)
               except:
                  pass        
