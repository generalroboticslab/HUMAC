...
class HideandSeekDataset(Dataset):
   ...
   def get(self, index):
      ...
      action = torch.zeros((6,1)) + 20 
      action_list = []
      
      if ind + self.step_ahead >= len(self.position_list):
         action1 = [self.position_list[-1][0]-self.position_list[ind][0],self.position_list[-1][1]-self.position_list[ind][1]]
      else:
         action1 = [self.position_list[ind+self.step_ahead][0]-self.position_list[ind][0],self.position_list[ind+self.step_ahead][1]-self.position_list[ind][1]]
      
      action_list.append(action1)

      for j in range(len(self.teammate_position_list)):
         if ind + self.step_ahead >= len(self.teammate_position_list[j]):
            action2 = [self.teammate_position_list[j][-1][0]-self.teammate_position_list[j][ind][0],self.teammate_position_list[j][-1][1]-self.teammate_position_list[j][ind][1]]
         else:
            action2 = [self.teammate_position_list[j][ind+self.step_ahead][0]-self.teammate_position_list[j][ind][0],self.teammate_position_list[j][ind+self.step_ahead][1]-self.teammate_position_list[j][ind][1]]
      ...
      all_position_list.append(self.position_list[ind])
         
      teammate_position = torch.zeros((2,3)) - 20
      for i in range(self.num_seekers-1):
         all_position_list.append(self.teammate_position_list[i][ind])
         for j in range(len(teammate_position)):
            teammate_position[j,i] = self.teammate_position_list[i][ind][j]
      

      index_list = list(range(len(action_list)))
      index_list = sorted(index_list,key = lambda x:(all_position_list[x][0] - all_position_list[0][0])**2 + (all_position_list[x][1] - all_position_list[0][1])**2)

      for i,index in enumerate(index_list):
         # print(index)
         action[2*i,:] = action_list[index][0]
         action[2*i+1,:] = action_list[index][1]
      ...
      # Calculate the extent of the image to center it at (0, 0)
         ...
         if (i == num_of_frame):
            for j in range(3):
               try:
                  if action[2*j] > -19: 
                     axs[0,i-1].scatter(action[2*j] - position[0], action[2*j+1] - position[1], c='orange', marker='+', s=60 - j* 20)
               except:
                  pass        
