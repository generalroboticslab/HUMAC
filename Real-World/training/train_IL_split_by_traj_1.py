...
from Model.Imitation_learning import CNN, train_on_one
    ...
    for seed_num in os.scandir(base_folder):
        ...
        for agent_id in range(0,num_seekers):
            ...
            if num_traj <= 45:
                ...
                # print(folder.name,seed_num.name)
                try:
...
model.fc = nn.Linear(512,2 + 3)
model = CustomResNet18(num_of_frames)
IL_state_dict = torch.load("Weights/IL_Long_split_by_traj_3frames_5step_ahead/model_epoch65.pth")["model_state_dict"]
model.load_state_dict(IL_state_dict)
...
save_base_directory = f"Weights/IL_Long_split_by_traj_{num_of_frames}frames_{step_ahead}step_ahead_tuning"
