...
def train(model,
    ...
    teammate_prediction_model = teammate_prediction_model.to(device)
    ...
    for epoch in range(num_epochs):
        ...
        for images, targets,_,num_teammate in train_loader:
            images, targets = images.to(device), targets.to(device)
            # Forward pass
            teammate_action_prediction = teammate_prediction_model(images)

            for i in range(len(images)):
                teammate_action_prediction[i][(num_teammate[i]-1)*2:] = 0

            outputs = model(images,teammate_action_prediction)
            # Compute the loss
        ...
        val_loss = 0.0
        with torch.no_grad():
            for images1, targets1,_,num_teammate1 in val_loader:
                images1, targets1 = images1.to(device), targets1.to(device)

                teammate_action_prediction1 = teammate_prediction_model(images1)

                for i in range(len(images1)):
                    teammate_action_prediction1[i][(num_teammate1[i]-1)*2:] = 0
                # Forward pass
                outputs1 = model(images1,teammate_action_prediction1)
...
def fine_tune(model,teammate_prediction_model,num_epochs,device,train_loader,val_loader,old_train_loader,old_val_loader,criterion,optimizer,checkpoint_dir,ratio):
    run = wandb.init(
        # set the wandb project where this run will be logged
        project="Multi-agent-HS",
        name = checkpoint_dir.split("/")[-1], 
    ...
    for epoch in range(num_epochs):
        ...
        for train_data,old_train_data in big_train_loader:
            ...
            images1, targets1,_,num_teammate = train_data
            ...
            old_images1, old_targets1,_, old_num_teammate = old_train_data
            old_images, old_targets = old_images1.to(device), old_targets1.to(device)
            old_teammate_action_prediction = teammate_prediction_model(old_images)

            for i in range(len(old_images)):
                old_teammate_action_prediction[i][(old_num_teammate[i]-1)*2:] = 0
            
            old_outputs = model(old_images,old_teammate_action_prediction)
        ...
        with torch.no_grad():
            for val_data,old_val_data in big_val_loader:
                if val_data is None:
                    break
                images2, targets2,_,num_teammate1 =  val_data
                images3, targets3,_,old_num_teammate1 =  old_val_data
                ...
                old_teammate_action_prediction1 = teammate_prediction_model(old_images1)

                for i in range(len(images1)):
                    old_teammate_action_prediction1[i][(old_num_teammate1[i]-1)*2:] = 0

                old_outputs1 = model(old_images1,old_teammate_action_prediction1)
