...
public class SeekerScript : MonoBehaviour
{
    ...
    void Start()
    {
        transform.position = new Vector3(UnityEngine.Random.Range(-19.5f,19.5f),0.5f,UnityEngine.Random.Range(-19f,-15.5f));
        speed = maxspeed;
        counter = 0;

        //set a random starting direction
        lastDirection = Random.insideUnitSphere;
        lastDirection.y = 0;
        lastDirection.Normalize();
        path = "/home/zhengranji/Desktop/Multi-agent-Hide-and-Seek/None crew version/test_data/Herustic_policy_test_random.txt";
        File.AppendAllText(path, "Seeker Initial Position:"+transform.position.ToString("F2")+"\n");
        File.AppendAllText(path, "Seeker Initial Direction:"+lastDirection.ToString("F2")+"\n");
    }
        ...
        if (!seeplayer) //didn't see hider
        {
        ...
        else //Know hider's last direction, move to hider's last show up position
            {
                ...
                if (see_obstacle)
                {
                    for (int i = 0; i < 6;i++)
                    {
                        Vector3 ray_d11 = rotate(Heading,Mathf.PI/180*10f*i,true);
                        Vector3 ray_d12 = rotate(Heading,Mathf.PI/180*10f*(i+1),true);
                        Vector3 ray_d13 = rotate(Heading,Mathf.PI/180*10f*(i-1),true);
                        if (!Physics.Raycast(transform.position, ray_d11, out hit, obstacledetectrange)
                        && !Physics.Raycast(transform.position, ray_d12, out hit, obstacledetectrange)
                        && !Physics.Raycast(transform.position, ray_d13, out hit, obstacledetectrange)
                        )
                        ...
                        Vector3 ray_d21 = rotate(Heading,Mathf.PI/180*10f*i,false);
                        Vector3 ray_d22 = rotate(Heading,Mathf.PI/180*10f*(i+1),false);
                        Vector3 ray_d23 = rotate(Heading,Mathf.PI/180*10f*(i-1),false);
                        if (!Physics.Raycast(transform.position, ray_d21, out hit, obstacledetectrange)
                        && !Physics.Raycast(transform.position, ray_d22, out hit, obstacledetectrange)
                        && !Physics.Raycast(transform.position, ray_d23, out hit, obstacledetectrange)
                        )
