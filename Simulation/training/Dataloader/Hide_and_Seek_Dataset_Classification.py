...
class HideandSeekDataset(Dataset):
   def __init__(self,folder_name,seeker_id,num_of_frame = 5,transform=transforms.ToTensor()):
   ...
   def get(self, index):
      ...
      return obs,action,position,human_control

   def num_of_human_control(self):
      return sum(1 for file in self.image_files if len(file) >= 5 and file[-5] == 'H')
   ...
   def __getitem__(self,ind):
      ...
      for i in range(num_frames):
         prev_index = index - i
         if prev_index < 0:
            obs_i, _, _, _ = self.get(last_ind)
         else:
            obs_i, _, _, _ = self.get(ind - i)
            last_ind = ind - i
      ...
      _,action, position,human_control = self.get(ind)

      return obs, action, position, human_control


   def plot(self,index):
      observation,action,position,human_control= self.__getitem__(index)
