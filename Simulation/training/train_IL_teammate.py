...
from Model.Imitation_learning_teammate import train
    ...
    if num_seekers >= 2:
        ...
        for seed_num in os.scandir(base_folder):
            for agent_id in range(num_seekers):
                dataset = HideandSeekDataset(os.path.join(base_folder,seed_num.name),agent_id,num_seekers,max_seeker_number = num_seekers_max)
...
model = CustomResNet18(num_of_frames,num_seekers_max-1)
...
save_base_directory = "Weights/teammate_prediction_short"
